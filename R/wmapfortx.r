wmapfortx <- function (chloropleth_map, geog_id,  variable, outline_map = NULL, 
          ...
          patch_height = 0.25, label_position = "right", verbose = F, plotly_legend=paste0(variable)) 
{
  if (!is.data.table(chloropleth_map)) {
    stop("The input for your chloropleth map needs to be a fortified spatial object as a data.table.")
  }
  chloropleth_map <- copy(chloropleth_map)
  outline_map <- copy(outline_map)
  data <- copy(data)
  if (!(geog_id %in% names(chloropleth_map))) 
    stop("That geographic ID does not appear to exist within your fortified object.")
  ...
  if (!is.null(data)) {
    ...
    if (!(variable %in% names(data))) 
      stop("That variable does not appear to exist within your data set.")
  ...
  for (select_dimension in map_dims) {
    ...
    if (discrete_scale == F) {
      ...
      map_plot <-ggplot(subset, aes(text = paste0("Country: ",geog_id, "<br>", plotly_legend, ": ", format(variable, digits=2) ) )) + geom_polygon(aes(x = long, 
