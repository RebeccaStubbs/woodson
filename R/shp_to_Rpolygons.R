...
shp_to_Rpolygons<-function(shp_dir,shp_layer,id_field,all_data=T){
  ...
  print(paste0("PSA: There is a new column in the @data slot called polygon_order. Remember that the attributes (values and IDs) for each ",
  ...
  if(all_data){
    if(!nrow(full_data)==rows_orig){stop("Whoah--the id_field you chose doesn't seem to be the primary key for the shapefile. 
                                         ...
                                         read.dbf function in the foregin library to read in the attributes alone.")}
